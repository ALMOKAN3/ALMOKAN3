...
مكتبات السي:
  ...
  3-(string.h)
    (functions):
      ...
      2-(strcmp();) (string) للمقارنة بين متغيرين من النوع
  4-(strings.h)
    (functions):
      1-(strcpy(السلسلة المراد نسخها، السلسلة التي ستحفظ النسخة);) لنسخ سلسلة أحرف
  5-(ctype.h)
    (functions):
  ...
  6-(stdlib.h)
    (functions):
      1-(malloc(حجم الفراغات المراد);) لتفريغ(تخصيص) الذاكرة ديناميكيا
      // ملاحظة :
        .قبل التشفيل (/.) لمعرفة إذا كان هنالك موقع من الذاكرة غير محرر (terminal) في الـ (valgrind) يمكن استخدام الكلمة
      2-(free(السلسلة التي تم تخزين عناصرها يدويا);) لتحريح الذاكرة المخصصة ديناميكيا
  7-(math.h)
    (functions):
  8-(time.h)
    (functions):

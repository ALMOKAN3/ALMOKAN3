...
أنواع المتغيرات في السي:
  ...
  7-(string) = ?? لهذا لها حالات خاصة دائما (cs50.h) للكلمات ولكنها ليست نوع أساسي و إنما تمنحه لنا مكتبة
  ...
  1-(stdio.h)
    (functions):
      ...
      2-(scanf(""),;) للإستقبال من المستخدم
        ...
        %b (bool)
        تكتب بين علامات التنصيص و يكتب اسم المتغير بعد الأقواس بعد وضع فاصلة
  2-(cs50.h) هي مكتبة خاصة
    (functions):
      1-(-int- i = get_int("");) لإستقبال قيمة عدد صحيح من المستخدم
      2-(-string- s = get_string("");) لإستقبال كلمة من المستخدم
      3-(-float- f = get_float("");) لإستقبال عدد عشري من المستخدم
      4-(-double- d = get_double("");) لإستقبال عدد عشري من المستخدم
      5-(-char- c = get_char("");) لإستقبال حرف أو رمز من المستخدم
      6-(-long- l = get_long("");) لإستقبال قيمة عدد صحيح من المستخدم
  3-(string.h)
    (functions):
